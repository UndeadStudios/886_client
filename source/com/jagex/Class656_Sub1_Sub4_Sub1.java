/* Class656_Sub1_Sub4_Sub1 - Decompiled by JODE
 * Visit http://jode.sourceforge.net/
 */
package com.jagex;
import java.util.Iterator;

public class Class656_Sub1_Sub4_Sub1 extends Class656_Sub1_Sub4
    implements Interface62
{
    boolean aBool12069;
    public Class532 aClass532_12070;
    Class553 aClass553_12071;
    boolean aBool12072 = true;
    
    boolean method16964() {
	return false;
    }
    
    boolean method16913(int i) {
	return false;
    }
    
    boolean method16914(int i) {
	return aBool12072;
    }
    
    boolean method16949() {
	return false;
    }
    
    public int method16909(byte i) {
	return aClass532_12070.method8805(274883650);
    }
    
    public int method16936(int i) {
	return aClass532_12070.method8807((byte) 69);
    }
    
    public void method18729(Class604 class604, int i) {
	aClass532_12070.method8806(class604, 1262401878);
    }
    
    public Class553 method16955(Class182 class182) {
	return aClass553_12071;
    }
    
    void method16918(Class182 class182, int i) {
	Class179 class179 = aClass532_12070.method8808(class182, 262144, false,
						       true, 1758931670);
	if (class179 != null) {
	    Class444 class444 = method10826();
	    Class428 class428 = method10818();
	    int i_0_ = (int) class428.aClass446_4807.aFloat4895 >> 9;
	    int i_1_ = (int) class428.aClass446_4807.aFloat4897 >> 9;
	    aClass532_12070.method8829(class182, class179, class444, i_0_,
				       i_0_, i_1_, i_1_, false, (byte) -16);
	}
    }
    
    boolean method16910(Class182 class182, int i, int i_2_, byte i_3_) {
	Class602 class602 = aClass532_12070.method8833((byte) 42);
	if (class602.aClass443_7920 != null)
	    return class182.method3414(i, i_2_, method10826(),
				       class602.aClass443_7920, (byte) -108);
	Class179 class179 = aClass532_12070.method8808(class182, 131072, false,
						       false, 2006311486);
	if (null == class179)
	    return false;
	return class179.method2954(i, i_2_, method10826(), false, 0);
    }
    
    public int method406(byte i) {
	return aClass532_12070.anInt7150 * -434636307;
    }
    
    public int method72(int i) {
	return 1886365003 * aClass532_12070.anInt7167;
    }
    
    public int method413(int i) {
	return aClass532_12070.anInt7154 * 287036693;
    }
    
    public void method395(int i) {
	/* empty */
    }
    
    public boolean method394(byte i) {
	return true;
    }
    
    public boolean method396(int i) {
	return aClass532_12070.method8810(-944357666);
    }
    
    public void method404(Class182 class182, short i) {
	aClass532_12070.method8813(class182, -1348865958);
    }
    
    public void method393(Class182 class182, int i) {
	aClass532_12070.method8812(class182, 733380782);
    }
    
    public int method77() {
	return aClass532_12070.anInt7150 * -434636307;
    }
    
    public int method71() {
	return aClass532_12070.anInt7150 * -434636307;
    }
    
    public boolean method405() {
	return true;
    }
    
    public int method399() {
	return 1886365003 * aClass532_12070.anInt7167;
    }
    
    public void method18730(Class604 class604) {
	aClass532_12070.method8806(class604, 1262401878);
    }
    
    boolean method16950() {
	return false;
    }
    
    public int method402() {
	return aClass532_12070.anInt7154 * 287036693;
    }
    
    public int method392() {
	return aClass532_12070.anInt7154 * 287036693;
    }
    
    public void method353() {
	/* empty */
    }
    
    void method16938(Class182 class182) {
	Class179 class179 = aClass532_12070.method8808(class182, 262144, false,
						       true, 121397757);
	if (class179 != null) {
	    Class444 class444 = method10826();
	    Class428 class428 = method10818();
	    int i = (int) class428.aClass446_4807.aFloat4895 >> 9;
	    int i_4_ = (int) class428.aClass446_4807.aFloat4897 >> 9;
	    aClass532_12070.method8829(class182, class179, class444, i, i,
				       i_4_, i_4_, false, (byte) -119);
	}
    }
    
    public boolean method289() {
	return aClass532_12070.method8810(-1407053374);
    }
    
    public int method16975() {
	return aClass532_12070.method8807((byte) 52);
    }
    
    public Class656_Sub1_Sub4_Sub1
	(Class546 class546, Class182 class182, Class40_Sub11 class40_sub11,
	 Class602 class602, int i, int i_5_, int i_6_, int i_7_, int i_8_,
	 boolean bool, int i_9_, int i_10_, int i_11_, int i_12_, int i_13_,
	 int i_14_) {
	super(class546, i_6_, i_7_, i_8_, i, i_5_, i_9_, i_10_);
	aClass532_12070
	    = new Class532(class182, class40_sub11, class602, i_11_, i_12_,
			   i_5_, this, bool, i_13_, i_14_);
	aBool12069 = class602.anInt7888 * 1700857535 != 0 && !bool;
	method16961(1, -16711936);
    }
    
    public int method401() {
	return aClass532_12070.anInt7154 * 287036693;
    }
    
    boolean method16948() {
	return false;
    }
    
    public void method408(Class182 class182) {
	aClass532_12070.method8813(class182, 126281276);
    }
    
    public void method409(Class182 class182) {
	aClass532_12070.method8813(class182, 1990640751);
    }
    
    public void method410(Class182 class182) {
	aClass532_12070.method8813(class182, -1899008130);
    }
    
    public void method411(Class182 class182) {
	aClass532_12070.method8812(class182, 733380782);
    }
    
    public void method412(Class182 class182) {
	aClass532_12070.method8812(class182, 733380782);
    }
    
    public void method397(Class182 class182) {
	aClass532_12070.method8812(class182, 733380782);
    }
    
    boolean method16934() {
	return aBool12072;
    }
    
    boolean method16933() {
	return aBool12072;
    }
    
    boolean method16929() {
	return aBool12072;
    }
    
    boolean method16931() {
	return aBool12072;
    }
    
    boolean method16932() {
	return aBool12072;
    }
    
    public void method407(Class182 class182) {
	aClass532_12070.method8813(class182, 1765993549);
    }
    
    public int method16927() {
	return aClass532_12070.method8805(2063101135);
    }
    
    public int method16935() {
	return aClass532_12070.method8805(1445201831);
    }
    
    public Class553 method16919(Class182 class182, short i) {
	return aClass553_12071;
    }
    
    Class567 method16917(Class182 class182, int i) {
	Class179 class179 = aClass532_12070.method8808(class182, 2048, false,
						       true, 1581551724);
	if (null == class179)
	    return null;
	Class444 class444 = class182.method3225();
	class444.method7154(method10826());
	class444.method7147((float) aShort11942, 0.0F, (float) aShort11939);
	Class428 class428 = method10818();
	Class567 class567 = Class293.method5303(aBool12069, (short) -17460);
	int i_15_ = (int) class428.aClass446_4807.aFloat4895 >> 9;
	int i_16_ = (int) class428.aClass446_4807.aFloat4897 >> 9;
	aClass532_12070.method8829(class182, class179, class444, i_15_, i_15_,
				   i_16_, i_16_, true, (byte) -114);
	Class602 class602 = aClass532_12070.method8833((byte) -38);
	if (null != class602.aClass443_7920) {
	    class179.method2953(class444, null, 0);
	    class182.method3371(class444, aClass211Array10869[0],
				class602.aClass443_7920);
	} else
	    class179.method2953(class444, aClass211Array10869[0], 0);
	if (null != aClass532_12070.aClass633_7152) {
	    Class185 class185 = aClass532_12070.aClass633_7152.method10350();
	    class182.method3412(class185);
	}
	aBool12072
	    = class179.method3017() || null != aClass532_12070.aClass633_7152;
	if (aClass553_12071 == null)
	    aClass553_12071
		= Class103.method1893((int) class428.aClass446_4807.aFloat4895,
				      (int) class428.aClass446_4807.aFloat4896,
				      (int) class428.aClass446_4807.aFloat4897,
				      class179, -650409126);
	else
	    Class460.method7588(aClass553_12071,
				(int) class428.aClass446_4807.aFloat4895,
				(int) class428.aClass446_4807.aFloat4896,
				(int) class428.aClass446_4807.aFloat4897,
				class179, -223648857);
	return class567;
    }
    
    void method16963(Class182 class182) {
	Class179 class179 = aClass532_12070.method8808(class182, 262144, false,
						       true, 1142798893);
	if (class179 != null) {
	    Class444 class444 = method10826();
	    Class428 class428 = method10818();
	    int i = (int) class428.aClass446_4807.aFloat4895 >> 9;
	    int i_17_ = (int) class428.aClass446_4807.aFloat4897 >> 9;
	    aClass532_12070.method8829(class182, class179, class444, i, i,
				       i_17_, i_17_, false, (byte) -128);
	}
    }
    
    boolean method16939(Class182 class182, int i, int i_18_) {
	Class602 class602 = aClass532_12070.method8833((byte) -5);
	if (class602.aClass443_7920 != null)
	    return class182.method3414(i, i_18_, method10826(),
				       class602.aClass443_7920, (byte) -115);
	Class179 class179 = aClass532_12070.method8808(class182, 131072, false,
						       false, 254758919);
	if (null == class179)
	    return false;
	return class179.method2954(i, i_18_, method10826(), false, 0);
    }
    
    public Class553 method16954(Class182 class182) {
	return aClass553_12071;
    }
    
    public int method83() {
	return aClass532_12070.anInt7150 * -434636307;
    }
    
    public void method200() {
	/* empty */
    }
    
    boolean method16965(Class182 class182, int i, int i_19_) {
	Class602 class602 = aClass532_12070.method8833((byte) -24);
	if (class602.aClass443_7920 != null)
	    return class182.method3414(i, i_19_, method10826(),
				       class602.aClass443_7920, (byte) -58);
	Class179 class179 = aClass532_12070.method8808(class182, 131072, false,
						       false, 1050920844);
	if (null == class179)
	    return false;
	return class179.method2954(i, i_19_, method10826(), false, 0);
    }
    
    boolean method16940(Class182 class182, int i, int i_20_) {
	Class602 class602 = aClass532_12070.method8833((byte) -20);
	if (class602.aClass443_7920 != null)
	    return class182.method3414(i, i_20_, method10826(),
				       class602.aClass443_7920, (byte) -126);
	Class179 class179 = aClass532_12070.method8808(class182, 131072, false,
						       false, 675087178);
	if (null == class179)
	    return false;
	return class179.method2954(i, i_20_, method10826(), false, 0);
    }
    
    public void method18731(Class604 class604) {
	aClass532_12070.method8806(class604, 1262401878);
    }
    
    public Class553 method16946(Class182 class182) {
	return aClass553_12071;
    }
    
    public Class553 method16953(Class182 class182) {
	return aClass553_12071;
    }
    
    public int method400() {
	return 1886365003 * aClass532_12070.anInt7167;
    }
    
    public boolean method403() {
	return true;
    }
    
    Class567 method16921(Class182 class182) {
	Class179 class179 = aClass532_12070.method8808(class182, 2048, false,
						       true, 1363477861);
	if (null == class179)
	    return null;
	Class444 class444 = class182.method3225();
	class444.method7154(method10826());
	class444.method7147((float) aShort11942, 0.0F, (float) aShort11939);
	Class428 class428 = method10818();
	Class567 class567 = Class293.method5303(aBool12069, (short) -22764);
	int i = (int) class428.aClass446_4807.aFloat4895 >> 9;
	int i_21_ = (int) class428.aClass446_4807.aFloat4897 >> 9;
	aClass532_12070.method8829(class182, class179, class444, i, i, i_21_,
				   i_21_, true, (byte) -56);
	Class602 class602 = aClass532_12070.method8833((byte) 25);
	if (null != class602.aClass443_7920) {
	    class179.method2953(class444, null, 0);
	    class182.method3371(class444, aClass211Array10869[0],
				class602.aClass443_7920);
	} else
	    class179.method2953(class444, aClass211Array10869[0], 0);
	if (null != aClass532_12070.aClass633_7152) {
	    Class185 class185 = aClass532_12070.aClass633_7152.method10350();
	    class182.method3412(class185);
	}
	aBool12072
	    = class179.method3017() || null != aClass532_12070.aClass633_7152;
	if (aClass553_12071 == null)
	    aClass553_12071
		= Class103.method1893((int) class428.aClass446_4807.aFloat4895,
				      (int) class428.aClass446_4807.aFloat4896,
				      (int) class428.aClass446_4807.aFloat4897,
				      class179, -650409126);
	else
	    Class460.method7588(aClass553_12071,
				(int) class428.aClass446_4807.aFloat4895,
				(int) class428.aClass446_4807.aFloat4896,
				(int) class428.aClass446_4807.aFloat4897,
				class179, -944276315);
	return class567;
    }
    
    public boolean method398() {
	return true;
    }
    
    boolean method16930() {
	return aBool12072;
    }
    
    static final void method18732(byte i) {
	Iterator iterator = client.aClass10_11248.iterator();
	while (iterator.hasNext()) {
	    Class525_Sub16_Sub15 class525_sub16_sub15
		= (Class525_Sub16_Sub15) iterator.next();
	    Class656_Sub1_Sub3_Sub4 class656_sub1_sub3_sub4
		= class525_sub16_sub15.aClass656_Sub1_Sub3_Sub4_11841;
	    class656_sub1_sub3_sub4.method18843(1, -1054320755);
	    if (class656_sub1_sub3_sub4.method18835((byte) -126)) {
		class525_sub16_sub15.method8755(-1933461091);
		class656_sub1_sub3_sub4.method18836(-14186507);
	    }
	}
    }
}
