/* Class441 - Decompiled by JODE
 * Visit http://jode.sourceforge.net/
 */
package com.jagex;

public class Class441 implements Interface6
{
    JS5 aClass458_4861;
    String aString4862;
    static String aString4863;
    
    public int method49(int i) {
	if (aClass458_4861.method7534(aString4862, -778236691))
	    return 100;
	return 0;
    }
    
    public Class27 method52() {
	return Class27.aClass27_277;
    }
    
    public Class27 method48(int i) {
	return Class27.aClass27_277;
    }
    
    public Class27 method51() {
	return Class27.aClass27_277;
    }
    
    public int method12() {
	if (aClass458_4861.method7534(aString4862, -914967206))
	    return 100;
	return 0;
    }
    
    public int method50() {
	if (aClass458_4861.method7534(aString4862, -978200776))
	    return 100;
	return 0;
    }
    
    Class441(JS5 class458, String string) {
	aClass458_4861 = class458;
	aString4862 = string;
    }
    
    public Class27 method47() {
	return Class27.aClass27_277;
    }
    
    public static Class606[] method7116(byte i) {
	return (new Class606[]
		{ Class606.aClass606_7967, Class606.aClass606_7966,
		  Class606.aClass606_7965, Class606.aClass606_7968,
		  Class606.aClass606_7964, Class606.aClass606_7963 });
    }
    
    static final void method7117(Class683 class683, int i) {
	Class679 class679 = (class683.aBool8665 ? class683.aClass679_8642
			     : class683.aClass679_8658);
	Class259 class259 = class679.aClass259_8623;
	Class245 class245 = class679.aClass245_8622;
	Class336.method5861(class259, class245, class683, 1584932955);
    }
    
    static final void method7118(Class683 class683, int i) {
	Class198_Sub13.aClass525_Sub30_9973.method16502
	    (Class198_Sub13.aClass525_Sub30_9973.aClass696_Sub45_10726,
	     (class683.anIntArray8661
	      [(class683.anInt8662 -= 1552651121) * 501271953]),
	     (byte) -41);
	Class650.method10715(-1758466106);
	client.aBool11084 = false;
    }
    
    public static void method7119(int i, int i_0_) {
	if (1777895575 * client.anInt11075 != i) {
	    client.anInt11097 = 0;
	    if (1777895575 * client.anInt11075 == 7 && 4 != i)
		Class585.method9662(1080614061);
	    if (6 == i || i == 3)
		Class517.method8635(-981884007);
	    if (i != 6 && Class208.aClass559_2261 != null) {
		Class208.aClass559_2261.method9381(1074357863);
		Class208.aClass559_2261 = null;
	    }
	    if (i == 1) {
		Class335.method5858((9 == client.anInt11075 * 1777895575
				     || 8 == 1777895575 * client.anInt11075
				     || (-750203937 * client.anInt11260
					 != 1089481059 * (Class138
							  .aClass631_1641
							  .anInt8176))),
				    (byte) -80);
		Class556_Sub1.method16483((short) 4378);
	    }
	    if (16 == i) {
		boolean bool;
		if (-1 == -1150509307 * Class138.aClass631_1641.anInt8177)
		    bool = (14 == 1777895575 * client.anInt11075
			    || 1777895575 * client.anInt11075 == 12);
		else
		    bool = (Class138.aClass631_1641.anInt8177 * -1150509307
			    != client.anInt11260 * -750203937);
		Class386.method6446(bool, -1792723034);
	    }
	    if (14 == i || i == 12) {
		if (!Class683.method11231(-23393322))
		    return;
	    } else if (i == 0
		       || 17 == i && client.anInt11075 * 1777895575 != 15) {
		if (!Class517.method8635(-981884007))
		    return;
	    } else if (i == 8)
		Class521.method8696((byte) 1);
	    if (Class522_Sub1.method16054(i, -2003100668)) {
		client.aClass507_11137.method8370(1182546960);
		client.aClass507_11137.aLong5637 = 5709647995284375069L;
	    }
	    if (i == 4 || 1 == i)
		Class11.method715(-801712206);
	    boolean bool = (19 == i || Class350_Sub1.method15842(i, 623712526)
			    || Class34.method960(i, (byte) 24));
	    boolean bool_1_
		= (client.anInt11075 * 1777895575 == 19
		   || Class350_Sub1.method15842(1777895575 * client.anInt11075,
						230840464)
		   || Class34.method960(1777895575 * client.anInt11075,
					(byte) 45));
	    if (bool_1_ != bool)
		Class94.aClass463_912.method7619(!bool, -312799294);
	    if (Class522_Sub1.method16054(i, 90790159) || i == 6 || 3 == i)
		Class501.aClass182_5564.method3166();
	    client.anInt11075 = 1898149671 * i;
	}
    }
    
    public static int method7120(byte i) {
	if (Class695.aBool8757 && Class96.aFrame1186 != null)
	    return 3;
	return client.aBool11220 ? 2 : 1;
    }
    
    static final void method7121(Class683 class683, byte i) {
	int i_2_ = (class683.anIntArray8661
		    [(class683.anInt8662 -= 1552651121) * 501271953]);
	Class96 class96 = Class111.method1979(-1606650359);
	Class525_Sub15 class525_sub15
	    = Class16.method767(Class412.aClass412_4268, class96.aClass6_1169,
				1977859884);
	class525_sub15.aClass525_Sub38_Sub2_10546.method16735(0, -420611154);
	int i_3_ = (class525_sub15.aClass525_Sub38_Sub2_10546.index
		    * -1133521593);
	class525_sub15.aClass525_Sub38_Sub2_10546.method16735(i_2_,
							      -1572228151);
	class525_sub15.aClass525_Sub38_Sub2_10546.method16602
	    (class683.aClass401_8651.anInt4134 * -1275733157, 1032291082);
	class683.aClass401_8651.aClass525_Sub16_Sub11_4133.method18422
	    (class525_sub15.aClass525_Sub38_Sub2_10546,
	     class683.aClass401_8651.anIntArray4132, (byte) 70);
	class525_sub15.aClass525_Sub38_Sub2_10546.method16617
	    ((class525_sub15.aClass525_Sub38_Sub2_10546.index
	      * -1133521593) - i_3_,
	     (byte) 7);
	class96.method1794(class525_sub15, (short) 32479);
    }
}
